...
/**
  ...
  *       192.168.66.194:9092 subscribe dc-data http://192.168.66.194:8088/rulemgr/ruleInstanceWithBindByCode?code=dis_excess_ampli_change hdfs://192.168.66.192:8020/tmp/dc-streaming/checkpoint/cumulative-amount-with-exception-time/
  ***/
object AmplitudeWithExceptionTimeNoDelay {
...
//      otherQuery.awaitTermination()
  ...
  case class Device(gatewayId: String, namespace: String, pointId: String, id: String, v: Double, s: String, t: Long, amp:String)
      ...
      deviceList.foreach(device => {
        ...
        if(state.getOption.isDefined){
          if(device.t > state.get._1){
            ...
            isCurrentException = !(((cur_value - prev_value)/prev_value <= amp.toDouble) && ((cur_value - prev_value)/prev_value >= amp.toDouble * (-1)))
